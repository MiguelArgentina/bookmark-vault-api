class Rack::Attack
  ...
  throttle("api/authenticated_user", limit: 60, period: 1.minute) do |req|
    next unless req.path.start_with?(API_PREFIX)

    next if req.post? && %w[/api/v1/login /api/v1/register /api/v1/refresh /api/v1/logout].include?(req.path)

    token = extract_bearer_token(req)
    sub = token && jwt_subject(token)
